...
# 출력 (Top 23)
OUT_TRAIN_PT = os.path.join(PT_DIR, "2018train_xgbPI_23.pt")
OUT_TEST_PT  = os.path.join(PT_DIR, "2018test_xgbPI_23.pt")

OUT_TXT = os.path.join(FEATURE_DIR, "2018_xgbPI_top23.txt")
...
# 📌 XGBoost PI
# ===============================
def xgb_permutation_importance(X, y):
    model = XGBClassifier(
        n_estimators=100,
        max_depth=6,
        learning_rate=0.1,
...
def main():
    ...
    # 2. PI
    feat_df, model = xgb_permutation_importance(X_train, y_train)
